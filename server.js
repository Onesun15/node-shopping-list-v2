...
app.post('/recipes', jsonParser, (req, res) => {
  const recipesFields = ['name', 'ingredients'];
  for (let i=0; i<recipesFields.length; i++) {
    //console.log(recipesFields[i]);
    const recipeName = recipesFields[i];
    console.log(recipeName);
    if (!(recipeName in req.body)) {
      const message = `Missing \`${recipeName}\` in request body`;
      console.error(message);
      return res.status(400).send(message);
    }
  }
